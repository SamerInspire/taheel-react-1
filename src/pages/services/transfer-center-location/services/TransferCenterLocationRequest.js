...
const TransferCenterLocationRequest = () => {
    ...
    console.log("centerLicenceNumber+_+_+_+_+_+_+", centerLicenceNumber)
    ...
    useEffect(async () => {
        ...
        if (!!centerLicenceNumber) {
            ...
            console.log('enewableLicenses', [].push({ licenceNumber: centerLicenceNumber }))
            setIsLoading(false);
        } else {
            let getCentersRs = ''
            ...
            setIsLoading(true);
            getCentersRs = await getCentersForFinal(email);
